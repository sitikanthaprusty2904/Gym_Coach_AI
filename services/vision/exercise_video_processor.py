...
from detectors.squart import SquatDetector
...
class VideoProcessorClass(VideoProcessorBase):
    ...
    def _draw_skeleton(self,img,landmarks):
        ...
        for lm in landmarks:
            if lm.visibility>0.7:
                cv2.circle(img,(int(lm.x*w),int(lm.y*h)),5,(0,0,255),-1)
    ...
    def _draw_no_pose_warnings(self,img):
        h,w=img.shape[:2]
        cv2.putText(img,"No Pose Detected!",(int(w*0.5)-100,int(h*0.1)),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2,cv2.LINE_AA)
        cv2.putText(img,"Please Face the camera",(int(w*0.5)-150,int(h*0.1)+30),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2,cv2.LINE_AA)
        # return img
